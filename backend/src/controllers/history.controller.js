...
const getHistory = asyncHandler(async (req, res) => {
  ...
  const cacheKey = `history:${userId}:days:${days}`;

  const cachedHistory = await redisClient.get(cacheKey);
  if (cachedHistory) {
    return res.status(200).json(
      new ApiResponse(200, JSON.parse(cachedHistory), "History fetched from cache")
    );
  }
  ...
  return res.status(200).json(
    new ApiResponse(200, responseData, `History fetched successfully`)
  );
});
